...
- [Taylor series in sexagesimal cuneiform](CUNEIFORM_TAYLOR.md) — the Taylor coefficients of eˣ, sin, cos, ln, arctan in base 60, why exactly the first seven terms of the exponential series are regular numbers, and the Basel problem Σ 1/n² = π²/6 on a clay tablet. Generated by `ideas/taylor_series.py`.
- [π, e, and transcendental expressions](CUNEIFORM_PI_E.md) — the constants and expressions like π^π and e^(π²) rendered in cuneiform, with notes on how a Babylonian scribe could have approximated them.
- [Four Old Babylonian tablets, verified in exact sexagesimal](CUNEIFORM_CORPUS_TABLETS.md) — Si.427 (oldest applied geometry), IM 67118 (the Pythagorean theorem worked and proved), YBC 11120 (circle area with π = 3), and TMS 3 from Susa (the sharper π = 3;7,30 = 25/8), each stored in the corpus and checked in exact base-60 arithmetic.
- [Scalar-representation side-channel leakage](CUNEIFORM_SIDE_CHANNEL.md) — whether a base-60 encoding of an ECC secret scalar leaks less to power analysis than binary. A pre-committed, exhaustive experiment; the answer is a clean no. Generated by `ideas/scalar_leakage_experiment.py`.
